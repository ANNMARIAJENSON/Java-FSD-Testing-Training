...
(118,'Ewan','McGregor','M'),(119,'Christian','Bale','M'),(120,'Maggie','Gyllenhaal','F'),
...
values(201,'Alfred','Hitchcock'),(202,'Jack','Clayton'),(203,'David','Lean'),(204,'Michael','Cimino'),
...
where gen_id in(select gen_id from genres where gen_id=1008);
...
where rev_id in(select rev_id from rating where rev_stars >= 8);

-- ---------------------5.Write a query in SQL to list all the information of the actors who played a role in the movie ‘Deliverance’------------
select * from actor where act_id in(select act_id from movie_cast where mov_id=917);
